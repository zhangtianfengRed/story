...
Shader "Toony Colors Pro 2/Examples BIRP/Cat Demo/Cat/Style 5"
{
	Properties
	{
		[TCP2HeaderHelp(Base)]
	...
	SubShader
	{
		...
		void surf(Input input, inout SurfaceOutputCustom output)
		{
			...
			// Shader Properties Sampling
			float4 __albedo = ( ApplyHSVGrayscale(TCP2_TEX2D_SAMPLE(_MainTex, _MainTex, input.texcoord0.xy).rgba, _albedo_sat) );
			...
			output.__sketchTexture = ( TCP2_TEX2D_SAMPLE(_SketchTexture, _SketchTexture, screenUV * _ScreenParams.zw * _SketchTexture_ST.xy + _SketchTexture_ST.zw).aaa );
			output.__sketchAntialiasing = ( 20.0 );
			output.__sketchThresholdScale = ( 1.0 );
			output.__sketchMin = ( _SketchMin );
			output.__sketchMax = ( _SketchMax );
		...
		inline half4 LightingToonyColorsCustom(inout SurfaceOutputCustom surface, half3 viewDir, UnityGI gi)
		{
			...
			ramp *= atten;
			half3 sketch = surface.__sketchTexture;
			half sketchThresholdWidth = surface.__sketchAntialiasing * fwidth(ndl);
			sketch = smoothstep(sketch - sketchThresholdWidth, sketch, clamp(saturate(ramp * surface.__sketchThresholdScale), surface.__sketchMin, surface.__sketchMax));
			...
			color.rgb *= lerp(surface.__sketchColor, half3(1,1,1), sketch.rgb);
