...
[AddComponentMenu("Dialogue/对白语音字幕触发器")]
public class DialogueVoiceSubtitleTrigger : MonoBehaviour
{
    [Header("显示层")]
    ...
    [Header("事件")]
    [InspectorName("播放开始时")]
    public UnityEvent onPlaybackStarted = new UnityEvent();
    ...
    public void Play()
    {
        ...
        if (overlay == null)
        {
            Debug.LogWarning("[DialogueVoiceSubtitleTrigger] No DialogueSubtitleOverlay found.", this);
            return;
        }
        ...
        bool started = overlay.Play(voiceClip, playbackCues, options, onPlaybackFinished.Invoke);
        if (started)
        {
    ...
    public void Stop()
    {
        DialogueSubtitleOverlay overlay = ResolveOverlay(false);
        if (overlay != null)
        {
            overlay.Stop();
        }
    ...
    private DialogueSubtitleOverlay ResolveOverlay(bool allowCreate = true)
    {
        ...
        return allowCreate && createOverlayWhenMissing ? DialogueSubtitleOverlay.GetOrCreate() : null;
    }
